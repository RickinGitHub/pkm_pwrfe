...
class AgentCore:
    ...
    def _call_knowledge(self, query: str) -> dict:
        """Route knowledge queries to correct op: filter, list, tags, or lookup."""
        low = query.strip().lower()
        # "filter [精华] [职场]" → op=filter, tags=["精华","职场"]
        ...
        # "list" or "list all" → op=list
        if low in ("list", "list all", "list docs"):
            return self._mcp.call("knowledge", {"op": "list"})
        # "tags" → op=tags
        if low in ("tags", "show tags", "list tags"):
...
def main() -> None:
    ...
    # Shared corpus loader with optional chunking
    ...
    agent.register_mcp("knowledge", KnowledgeServer(corpus_loader))
    agent.register_mcp(
        "hybrid_knowledge",
        HybridKnowledgeServer(corpus_loader, embedder=embedder),
    )
